class VideoMirror {
    ...
    startProcessing() {
        ...
        const processFrame = () => {
            // Only process canvas-based filters when canvas is visible
            // and video has valid dimensions
            if (
                ...
                // Sync canvas dimensions in case they drifted
                if (
                    this.canvas.width !== this.video.videoWidth ||
                    this.canvas.height !== this.video.videoHeight
                ) {
    ...
    applyFilter() {
        ...
        // Guard: skip if canvas has no area (avoids getImageData IndexSizeError)
        if (w === 0 || h === 0) return;
    ...
    setFilter(filter) {
        ...
        if (filter === 'normal') {
            ...
            // Canvas-based filter: ensure canvas dimensions are valid before showing
            if (
                this.video.videoWidth > 0 &&
                this.video.videoHeight > 0 &&
                (this.canvas.width !== this.video.videoWidth ||
